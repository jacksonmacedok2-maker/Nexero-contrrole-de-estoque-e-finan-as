        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium italic">
              Gestão operacional centralizada em tempo real.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {hasSales ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">
                  Aguardando Volume de Dados
                </p>
                <p className="text-[10px] text-slate-500 italic max-w-xs font-medium">
                  As projeções estatísticas serão habilitadas após a sincronização das primeiras vendas.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-6">
              Status da Instância
            </h3>
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700/30 rounded-2xl transition-all hover:translate-x-1">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-800 dark:text-slate-300 uppercase tracking-widest">
                    Sincronia Local
                  </p>
                  <p className="text-[9px] text-slate-500 font-bold uppercase">
                    Ativa via Web Worker
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700/30 rounded-2xl transition-all hover:translate-x-1">
              ...
              {recentOrders.length > 0 ? (
                recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors group">
                    <td className="px-8 py-5 text-xs font-black text-brand-600">
                      #{order.code || order.id.substring(0, 8).toUpperCase()}
                    </td>
                    <td className="px-8 py-5 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight">
                      {order.clients?.name || 'Venda Avulsa'}
                    </td>
                    <td className="px-8 py-5 text-[10px] text-slate-500 font-black uppercase tracking-widest">
                      {formatDate(order.created_at)}
                    </td>
                    <td className="px-8 py-5 text-right text-sm font-black text-slate-900 dark:text-white group-hover:scale-105 transition-transform">
